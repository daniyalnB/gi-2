...
const FormElement = (props) => {
  ...
  const refundOrder = () => {
    ...
    const payload = {
			orderId: 0,
			subscriptionReceiptId: props.refundDetails.subscriptionReceiptId,
    ...
    refundOrderOrSubscriptionAmount(stringified).subscribe((response) => {
			if (response.response.Requested_Action) {
				history.push(`/gi-team/view-subscription/${props.refundDetails.customerId}`);
      }
      else {
...
const RefundSubscription = (props) => {
        ...
        if (response.response.Requested_Action) {
          ...
          if (x == undefined) {
            history.push("/gi-team/subscriptions");
					...
					const info = ({
						...
						address: x.ownerinfo.streetaddress,
						phone: x.ownerinfo.phonenumber,
          ...
          setUserInfo(info);
					setRefundDetails({
						...refundDetails,
						customerId: x.id,
						subscriptionReceiptId: props.location.state.history.id,
						purchaseType:
							props.location.state.history.planname === "StandardPlan" ? "Standard Plan (Monthly) Subscription" :
							props.location.state.history.planname === "PlusPlan" ? "Plus Plan (Monthly) Subscription" :
							props.location.state.history.planname === "ProfessionalPlan" ? "Pro Plan (Monthly) Subscription" :
							props.location.state.history.planname === "EnterprisePlan" ? "Enterprise Plan (Monthly) Subscription" :
							props.location.state.history.planname === "StandardPlanAnnual" ? "Standard Plan (Annual) Subscription" :
							props.location.state.history.planname === "PlusPlanAnnual" ? "Plus Plan (Annual) Subscription" :
							props.location.state.history.planname === "ProfessionalPlanAnnual" ? "Pro Plan (Annual) Subscription" :
							props.location.state.history.planname === "EnterprisePlanAnnual" ? "Enterprise Plan (Annual) Subscription" : "",
						price: props.location.state.history.chargedamountincents,
					});
					setPriceLimit(props.location.state.history.chargedamountincents);
  ...
  const handlecardshow = () => setcardpopupshow(true);
	...
	return (
		<>
			<Helmet>
				<title> 
					Refund Subscription - Actionable Insights Admin
				</title>
			</Helmet>
			<Modal 
        show={cardpopupshow} 
...
export default withRouter(RefundSubscription);